...
## h2================short rules for working with this repository for developers================
...
+ ul 2) Закрываете консоль и открываете ее в созданной после клонирования папке (или cd test_it_frontend). Создаете ветку с названием компонента который будете делать, например: 
  + ul 2.1) git checkout -b Home
  + ul 2.2) Чтобы установить все модули, на уровне с package.json запускаете консоль и команда:
npm i
  + ul 2.3) Ознакомьтесь с установленными модулями в package.json, и старайтесь ими обойтись, например для графиков это react-chartjs-2, для прогресс бара react-sweet-progress.
+ ul 3) Работаете в ветке например (Home), первая отправка файлов на репозиторий: 
  + ul 3.1) git add .
  + ul 3.2) git commit -m "some text"
  + ul 3.3) git push --set-upstream origin Home       (при повторных отправках на этом шаге просто - git push) 
